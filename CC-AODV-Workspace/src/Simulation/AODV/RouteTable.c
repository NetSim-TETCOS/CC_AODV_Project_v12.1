	...
	NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if (table->Lifetime < pstruEventDetails->dEventTime + AODV_ACTIVE_ROUTE_TIMEOUT)
		return 1;
}
/**
	...
	NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	while(table)
	{
		...
		table=LIST_NEXT(table);
		fp = fopen("AODV_LOG.txt", "a+");
		if (fp) 
		{ 
			fprintf(fp, "\n%d\t\t%d", pstruEventDetails->nDeviceId, AODV_DEV_VAR(pstruEventDetails->nDeviceId)->ncounter);
		fclose(fp);
		}
	}
	return 1;
